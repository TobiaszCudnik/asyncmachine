...
describe('asyncmachine', function() {
  ...
  it('should throw when activating an unknown state', function() {
    let { machine } = this
    let func = () => {
  ...
  it('should allow to define a new state', function() {
    let example = <any>machine(['A'])
    example.A = {}
    example.register('A')
    example.add('A')
    expect(example.is()).eql(['A'])
  })

  it('should allow to get relations of a state', function() {
    let example = machine<'A' | 'B'>({
      A: {
        add: ['B'],
        auto: true
      },
    ...
    expect(example.getRelationsOf('A')).eql([StateRelations.ADD])
  })

  it('should allow to get relations between 2 states', function() {
    let example = <any>machine({
      A: {
        add: ['B'],
        require: ['C'],
        auto: true
    ...
    expect(example.getRelationsOf('A', 'B')).eql([StateRelations.ADD])
  })
  ...
  describe('when one state blocks another', function() {
    beforeEach(function() {
      this.log = []
      this.machine = new FooMachine(['A', 'B'])
      this.machine.id('').logLevel(3)
      this.machine.def_log_handler_off = true
    ...
    describe('and they are set simultaneously', function() {
      ...
      it('should explain the reason in the log', function() {
        expect(this.log).to.contain('[rel:drop] D by C')
      })
  ...
  describe('when state is changed', function() {
    ...
    describe('and transition is canceled', function() {
      ...
      describe('when activating a new state', function() {
        beforeEach(function() {
    ...
    describe('and active state is also the target one', function() {
      it('should trigger self transition at the very beginning', function() {
        ...
        let order = [this.machine.A_A, this.machine.any_B, this.machine.B_enter]
        assert_order(order)
      })
    ...
    describe('should trigger events', function() {
      beforeEach(function() {
        ...
        // emitter event
        this.machine.on('tick', this.change)
        this.machine.on('transition-cancelled', this.cancelTransition)
        this.machine.set(['A', 'B'])
        this.machine.add(['C'])
        this.machine.add(['D'])
      })
  ...
  describe('Events', function() {
    ...
    describe('clock', function() {
      ...
      it('should tick when activating a new state', function() {
        this.machine.set('A')
        expect(this.machine.clock('A')).to.be.eql(1)
      })

      it('should tick when activating many new states', function() {
        this.machine.set(['A', 'B'])
        expect(this.machine.clock('A')).to.be.eql(1)
        expect(this.machine.clock('B')).to.be.eql(1)
      })

      it("shouldn't tick when activating an already active state", function() {
        this.machine.set('A')
        this.machine.set('A')
        expect(this.machine.clock('A')).to.be.eql(1)
      })

      it('should tick for Multi states when activating an already active state', function() {
        this.machine.A.multi = true
        // A is already active
        expect(this.machine.clock('A')).to.be.eql(1)
        this.machine.set('A')
        this.machine.set('A')
        // the clock should be incremented by +2
        expect(this.machine.clock('A')).to.be.eql(3)
      })
    })
